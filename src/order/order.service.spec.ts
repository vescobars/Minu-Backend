...
describe('OrderService', () => {
  ...
  it('create should return a new order', async () => {
    ...
    const storedOrder: OrderEntity = await repository.findOne({
  ...
  it('update should throw an exception for an invalid order', async () => {
    ...
    await expect(() => service.update('0', order)).rejects.toHaveProperty(
      'message',
      'The order with the given id was not found',
    );
  });
  ...
  it('delete should remove a order', async () => {
    ...
    const deletedOrder: OrderEntity = await repository.findOne({
      where: { id: order.id },
    });
