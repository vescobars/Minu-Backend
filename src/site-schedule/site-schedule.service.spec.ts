...
describe('SiteScheduleService', () => {
  ...
  beforeEach(async () => {
    ...
    siteRepository = module.get<Repository<RestaurantSiteEntity>>(
  ...
  it('addScheduleSite should add an schedule to a site', async () => {
    ...
    const result: RestaurantSiteEntity = await service.addScheduleSite(
      newSite.id,
      newSchedule.id,
    );
  ...
  it('addScheduleSite should thrown exception for an invalid schedule', async () => {
    ...
    await expect(() =>
      service.addScheduleSite(newSite.id, '0'),
  ...
  it('addScheduleSite should throw an exception for an invalid site', async () => {
    ...
    await expect(() =>
      service.addScheduleSite('0', newSchedule.id),
    ).rejects.toHaveProperty(
      'message',
      'The site with the given id was not found',
    );
  });

  it('findScheduleBySiteIdScheduleId should return schedule by site', async () => {
    const schedule: ScheduleEntity = schedulesList[0];
    const storedSchedule: ScheduleEntity =
      await service.findScheduleBySiteIdScheduleId(site.id, schedule.id);
  ...
  it('findScheduleBySiteIdScheduleId should throw an exception for an invalid schedule', async () => {
    await expect(() =>
      service.findScheduleBySiteIdScheduleId(site.id, '0'),
    ).rejects.toHaveProperty(
      'message',
      'The schedule with the given id was not found',
    );
  ...
  it('findScheduleBySiteIdScheduleId should throw an exception for an invalid site', async () => {
    const schedule: ScheduleEntity = schedulesList[0];
    await expect(() =>
      service.findScheduleBySiteIdScheduleId('0', schedule.id),
    ).rejects.toHaveProperty(
      'message',
      'The site with the given id was not found',
    );
  ...
  it('findScheduleBySiteIdScheduleId should throw an exception for an schedule not associated to the site', async () => {
    ...
    await expect(() =>
      service.findScheduleBySiteIdScheduleId(site.id, newSchedule.id),
    ).rejects.toHaveProperty(
      'message',
      'The schedule with the given id is not associated to the site',
    );
  });

  it('findSchedulesBySiteId should return schedules by site', async () => {
    const schedules: ScheduleEntity[] = await service.findSchedulesBySiteId(
      site.id,
    );
  ...
  it('findSchedulesBySiteId should throw an exception for an invalid site', async () => {
    await expect(() =>
      service.findSchedulesBySiteId('0'),
    ).rejects.toHaveProperty(
      'message',
      'The site with the given id was not found',
    );
  ...
  it('associateSchedulesSite should update schedules list for a site', async () => {
    ...
    const updatedSite: RestaurantSiteEntity =
      await service.associateSchedulesSite(site.id, [newSchedule]);
    expect(updatedSite.schedules.length).toBe(1);

    expect(updatedSite.schedules[0].day).toBe(newSchedule.day);
    expect(updatedSite.schedules[0].opening_hour).toBe(
      newSchedule.opening_hour,
    );
    expect(updatedSite.schedules[0].closing_hour).toBe(
      newSchedule.closing_hour,
    );
  });
  ...
  it('associateSchedulesSite should throw an exception for an invalid site', async () => {
    ...
    await expect(() =>
      service.associateSchedulesSite('0', [newSchedule]),
    ).rejects.toHaveProperty(
      'message',
      'The site with the given id was not found',
    );
  ...
  it('associateSchedulesSite should throw an exception for an invalid schedule', async () => {
    ...
    await expect(() =>
      service.associateSchedulesSite(site.id, [newSchedule]),
    ).rejects.toHaveProperty(
      'message',
      'The schedule with the given id was not found',
    );
  ...
  it('deleteScheduleSite should remove an schedule from a site', async () => {
    ...
    const storedSite: RestaurantSiteEntity = await siteRepository.findOne({
      where: { id: site.id },
      relations: ['schedules'],
    });
    const deletedSchedule: ScheduleEntity = storedSite.schedules.find(
      (a) => a.id === schedule.id,
    );
  ...
  it('deleteScheduleSite should thrown an exception for an invalid schedule', async () => {
    await expect(() =>
      service.deleteScheduleSite(site.id, '0'),
    ).rejects.toHaveProperty(
      'message',
      'The schedule with the given id was not found',
    );
  ...
  it('deleteScheduleSite should thrown an exception for an invalid site', async () => {
    const schedule: ScheduleEntity = schedulesList[0];
    await expect(() =>
      service.deleteScheduleSite('0', schedule.id),
    ).rejects.toHaveProperty(
      'message',
      'The site with the given id was not found',
    );
  ...
  it('deleteScheduleSite should thrown an exception for an non asocciated schedule', async () => {
    ...
    await expect(() =>
      service.deleteScheduleSite(site.id, newSchedule.id),
    ).rejects.toHaveProperty(
      'message',
      'The schedule with the given id is not associated to the site',
    );
