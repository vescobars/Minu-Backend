...
export class SiteScheduleService {
  ...
  async addScheduleSite(
    ...
    const site: RestaurantSiteEntity =
      await this.restaurantSiteRepository.findOne({
        where: { id: siteId },
        relations: [
          'orders',
          'tables',
  ...
  async findScheduleBySiteIdScheduleId(
    siteId: string,
    scheduleId: string,
    ...
    const siteSchedule: ScheduleEntity = site.schedules.find(
      (e) => e.id === schedule.id,
    );
